...
class PerformanceOptimizer:
    ...
    def _analyze_code_performance(self) -> Dict[str, Any]:
        ...
        try:
            for file_path in self.project_path.rglob("*.c"):
                self._scan_file_performance(file_path, performance_patterns, issues, opportunities)
        ...
        return {
            ...
            "total_files_scanned": len(list(self.project_path.rglob("*.[ch]")) + 
                                              list(self.project_path.rglob("*.cpp")) +
                                              list(self.project_path.rglob("*.cc")))
        }
    
    def _scan_file_performance(self, file_path: Path, patterns: Dict[str, List[str]], 
                             issues: List[Dict], opportunities: List[Dict]):
                ...
                for pattern in pattern_list:
                    ...
                    for match in matches:
                        ...
                        opportunity = {
                            ...
                            "category": category,
                            "code": match.group(0),
                            "suggestion": self._get_arm_optimization_suggestion(category)
                        }
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any], 
        ...
        if opportunities:
            simd_opportunities = [o for o in opportunities if o["category"] == "simd_candidates"]
            if simd_opportunities:
                recommendations.append(
                    f"💡 {len(simd_opportunities)} loops identified for ARM NEON vectorization"
                )
            
            fp_opportunities = [o for o in opportunities if o["category"] == "floating_point"]
            if fp_opportunities:
                recommendations.append(
        ...
        if build_analysis.get("optimization_level") not in ["O2", "O3"]:
            recommendations.append("⚡ Enable -O3 optimization for better ARM performance")
