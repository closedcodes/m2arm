...
class M2ARMCore:
    ...
    def scan_project(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Scan completed. Found {len(results.get('issues', []))} potential issues"
        )
        return results
    ...
    def full_migration_workflow(
        self, target_arch: str = "arm64", compile_targets: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if compile_targets is None:
            compile_targets = [
                target_arch,
                "amd64",
            ]  # Always include x86 for compatibility
