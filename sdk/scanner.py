...
class CodeScanner:
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        logger.info(f"Scan completed: {results['scanned_files']}/{results['total_files']} files")
        return results
    ...
    def _get_source_files(self) -> List[Path]:
        ...
        for file_path in self.project_path.rglob('*'):
            if (file_path.is_file() and 
                file_path.suffix.lower() in self.scannable_extensions and
                ...
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        issue = {
                            ...
                            "suggestion": self._get_suggestion(category, match.group(0))
                        }
                        issues.append(issue)
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        high_severity_count = sum(1 for issue in results["issues"] if issue["severity"] == "high")
        
        if issue_count == 0:
            recommendations.append("✅ No obvious x86-specific code detected")
        else:
            recommendations.append(f"🔍 Found {issue_count} potential compatibility issues")
            
            if high_severity_count > 0:
                recommendations.append(f"⚠️  {high_severity_count} high-severity issues require immediate attention")
        
        # Build system recommendations
        build_systems = [bs["system"] for bs in results["build_systems"]]
        if "cmake" in build_systems:
            recommendations.append("📋 CMake detected - review CMakeLists.txt for architecture-specific settings")
        if "make" in build_systems:
            recommendations.append("📋 Makefile detected - review for architecture-specific compiler flags")
        
        # Dependency recommendations
        dep_count = len(results["dependencies"])
        if dep_count > 0:
            recommendations.append(f"📦 {dep_count} dependencies found - verify ARM compatibility")
