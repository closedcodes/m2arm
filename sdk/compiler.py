...
class CrossCompiler:
    ...
    def compile_all(self, target_list: List[str]) -> Dict[str, Any]:
        ...
        for target in target_list:
            try:
                ...
                results["build_results"][target] = {"success": False, "error": str(e)}
                results["failed_builds"] += 1
    ...
    def _compile_target(self, target: str) -> Dict[str, Any]:
        ...
        if build_type == "go":
            ...
            return {"success": False, "error": f"No supported build system found"}
    ...
    def _compile_cmake(self, target: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            configure_result = subprocess.run(
                cmake_args, capture_output=True, text=True, timeout=120
            )
    ...
    def _compile_cargo(self, target: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not rust_target:
            return {"success": False, "error": f"No Rust target mapping for {target}"}

        try:
            # Add target if not installed
            subprocess.run(
                ["rustup", "target", "add", rust_target], capture_output=True
            )
    ...
    def validate_toolchain(self, target: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                [config["cc"], "--version"], capture_output=True, timeout=10
            )
        ...
        try:
            result = subprocess.run(
                [config["cxx"], "--version"], capture_output=True, timeout=10
            )
            if result.returncode != 0:
